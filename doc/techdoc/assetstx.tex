...
\subsection{Support For Ownership Preassets}\label{sec:docsuppown}

Ownership of objects, propositions and negated propositions
will be originally justified by a publication supporting the ownership preasset at
a given term address.
What {\defin{support}} means differs in the three cases.

Let $h$ be a theory identifier (an optional hash value),
$\Delta$ be a document
and $\alpha$ be a term address.
\begin{itemize}
\item We say $(h,\Delta)$ {\defin{supports}}
      the preasset ${\constr{OwnsObj}}(\beta,p)$ at $\alpha$
      if
...
\item We say $(h,\Delta)$ {\defin{supports}}
      the preasset ${\constr{OwnsProp}}(\beta,p)$ at $\alpha$ if
...
\item We say $(h,\Delta)$ {\defin{supports}}
      the preasset ${\constr{OwnsNegProp}}$ at $\alpha$ if
      there is a proof ${\constr{DocPfOf}}(s,\cD)$ in $\Delta$
      and a proposition $t$ where $s$ is either $\neg t$ or $t\to\bot$\footnote{Here $\bot$ is $\forall_o x_0$ and $\neg$ is $\lambda_o x_0\to\bot$.}
      and
      $\alpha$ is either the term address 
      of the proposition $t$ in the theory with theory identifier $h$.\footnote{Negated proposition ownership is not given to pure propositions. The only reason for negated proposotion ownership is to collect bounties. Bounties only make sense for propositions in a theory. (Note that every proposition is provable in an inconsistent theory, and so for every pure proposition there is some theory in which the proposition is provable.)}
\end{itemize}

Let $o$ be an {\type{addr\_preasset}} list (transaction outputs),
$u$ be an ownership preasset and $\alpha$ be a term address.
We say $o$ {\defin{supports}} $u$ at $\alpha$
if there is some 
$(\delta,(\omega,{\constr{DocPublication}}(\gamma,\nu,h,\Delta)))\in o$
where $(h,\Delta)$ supports $u$ at $\alpha$.
This notion of support will be used when we give the conditions
...
can be used to obtain term addresses which may support the ownership assets.
